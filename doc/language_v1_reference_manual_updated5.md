...
- **fixed arrays** are core (`Array(T,N)`)
...
## 7.1 `Array(T,N)` is a core language type constructor
...
```forth
const LUT = Array(u16,4){ 10 20 30 40 };
```
...
## 8.3 `&[ ... ]` and `&![ ... ]` (scoped borrows for Arrays and Regions)
...
When the top of stack is `Array(T,N)`:
...
- `&[  block ] : ( S Array(T,N) -- S Array(T,N) R... )`
- `&![ block ] : ( S Array(T,N) -- S Array(T,N) R... )`

**Block typing rule:**
At block entry, the compiler pushes the slice on top of the stack. The block must consume it:

- Block entry: `( S Array(T,N) Slice(T)    -- ... )`   (or `SliceMut(T)`)
- Block exit:  `( S Array(T,N) R...       )`           (slice must be gone)
...
```forth
: init-frame ( Array(u8,16) -- Array(u8,16) )
  &![ => s
       s 0 0xAA set
       s 1 0x55 set
     ]
...
type DutyTable = Array(Percent, 16);
```
...
Registers can be declared as arrays by adding `[N]` to the name:

```forth
register-map GPIO
  0x200 PINCFG[40] u32 rw volatile
end;
```
...
```forth
0x100 CH[4] TIMER_CH volatile stride=0x20
```
...
Channels are stdlib/platform types `Chan(T)`.
