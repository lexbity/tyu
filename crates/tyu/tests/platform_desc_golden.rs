...
fn hash_invariant_under_formatting() {
    ...
    // format gained the per-aperture scratch flag (P8 audit), when it gained
    // the `[verification]` profile grants (P3 slice), and when the amended
    // §6.4 dropped the declared `N_main` grant (format v6 — N_main is derived
    // from the runtime binary's geometry and hashes nothing here): the ISR
    // grant still folds into the hash so a budget change invalidates cached
    // artifacts exactly as a board change would.
    eprintln!("HASH_BASE={:#018x}", ha);
    assert_eq!(ha, 0xc5aa_6dde_3c6d_227c);
}
...
fn hash_changes_on_semantic_edit() {
    ...
    assert_eq!(h_base, 0xc5aa_6dde_3c6d_227c);
    assert_eq!(h_flipped, 0x70a1_9719_a348_e02d);
}
