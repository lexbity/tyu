...
: check-xor ( -- )
  # xor: write-1-invert XORs into SETXOR (RMW xor/eor). Writing the same
...
: mmio-strategies-arm-run ( -- )
  # w1s: write-1-set ORs into SETBITS (RMW orr).
...
export { mmio-strategies-arm-run };
end;