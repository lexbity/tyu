...
impl<'a> RiscVBackend<'a> {
    ...
    pub(crate) fn emit_modinfo_section(&mut self) -> Result<(), CodegenError> {
        ...
        let mut aperture_entries = [lmod::modinfo::ApertureUseEntry {
            name_hash: 0,
            size: 0,
            aperture_id: 0,
            access_mask: 0,
...
/// RISC-V backend (design doc §5.6 matrix). RISC-V has only bus apertures;
/// emulated-aperture combos are structurally `Unsupported`. Loads are plain
...
pub fn mmio_cell(
    ...
    match kind {
        ApertureKind::Emulated => StrategyCell::Unsupported {
            reason: "no emulated apertures on RISC-V",
        },
