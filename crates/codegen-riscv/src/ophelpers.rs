...
pub fn write_u32(out: &mut dyn Output, v: u32) {
    ...
    if v == 0 { buf[0] = b'0'; n = 1; }
        ...
        let mut u = v;
        while u > 0 && n < buf.len() { buf[n] = b'0' + (u % 10) as u8; n += 1; u /= 10; }
        buf[..n].reverse();
    }
    out.write(&buf[..n]);
}

pub fn write_hex(out: &mut dyn Output, v: u32) {
    let mut buf = [0u8; 12];
    buf[0] = b'0'; buf[1] = b'x';
    let mut n = 2usize;
...
pub fn slice_span<'a>(src: &'a [u8], span: Span) -> &'a [u8] {
    let s = span.start as usize; let e = span.end as usize;
    if s < e && e <= src.len() { &src[s..e] } else { &[] }
}
