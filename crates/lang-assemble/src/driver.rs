...
pub fn run(config: &Config) -> i32 {
    ...
    let path = config.assembler_path
        .unwrap_or_else(|| default_assembler_path(spec.assembler));

    match spec.assembler {
        AssemblerKind::Fasm => {
            run_with_driver(config, &FasmDriver { path })
        }
        AssemblerKind::GasArm | AssemblerKind::GasRiscV => {
            let _ = diag::error_simple(2005, b"assembler backend not yet implemented for this target");
            2
        }
    }
}
