...
mod tests {
    ...
    fn hash_collision_detected_via_direct_injection() {
        ...
        let colliding_hash = fnv1a_u64(b"first");
    ...
    fn register_from_modinfo_populates_map() {
        ...
        let mut buf = [0u8; 768];
        let size = lmod::modinfo::encode_into(&mut buf, b"MyMod", &exports, &[], 42).unwrap();
    ...
    fn register_from_modinfo_detects_duplicate() {
        ...
        let mut buf = [0u8; 256];
        let size = lmod::modinfo::encode_into(&mut buf, b"M", &exports, &[], 0).unwrap();
