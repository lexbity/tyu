    ...
    pub fn emit_prelude(&mut self) -> Result<(), CodegenError> {
        match self.mode {
            ...
            AsmMode::Object => {
                ...
                // P6: the aperture-base literal sites reference these extern
                // symbols; the pack binds them, the loader re-derives them.
                for i in 0..self.mmio_aperture_count {
                    let w = &self.mmio_apertures[i];
                    if w.reloc_isa.is_some() {
                        self.out.write(b"\t.extern __lang_aperture_");
