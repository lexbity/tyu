...
fn arm_dup_adjusts_by_8() {
    ...
    assert!(out.contains("subs r4, r4, #8"), "dup must adjust r4 by 8 down, got: {out}");
    assert!(out.contains("adds r4, r4, #8"), "dup must adjust r4 by 8 up, got: {out}");
}
...
fn arm_drop_adjusts_by_8() {
    ...
    assert!(out.contains("subs r4, r4, #8"), "drop must adjust r4 by 8, got: {out}");
}
...
fn arm_cmp_uses_it_block() {
    ...
    assert!(out.contains("itt") || out.contains("ite"), "cmp must use IT block, got: {out}");
}
...
fn arm_sub_emits_subs_sbc() {
    let out = emit(&single_block_word(
        sig_0_1(TY_I64),
        &[OpKind::ConstI64(5), OpKind::ConstI64(3), OpKind::SubI64, OpKind::Drop { ty: TY_I64 }, OpKind::Drop { ty: TY_I64 }],
    ));
    assert!(out.contains("subs"), "sub must emit subs, got: {out}");
    assert!(out.contains("sbc"), "sub must emit sbc (borrow), got: {out}");
}
...
fn arm_not_emits_cmp_ite() {
    let out = emit(&single_block_word(
        sig_0_1(TY_I64),
        &[OpKind::ConstI64(0), OpKind::NotBool, OpKind::Drop { ty: TY_I64 }, OpKind::Drop { ty: TY_I64 }],
    ));
    assert!(out.contains("itt") || out.contains("ite"),
...
fn arm_swap_uses_strd_ldrd() {
    ...
    assert!(out.contains("ldrd") || out.contains("strd"), "swap must use ldrd/strd, got: {out}");
}
...
fn arm_mul_emits_muls() {
    let out = emit(&single_block_word(
        sig_0_1(TY_I64),
        &[OpKind::ConstI64(3), OpKind::ConstI64(7), OpKind::MulI64, OpKind::Drop { ty: TY_I64 }, OpKind::Drop { ty: TY_I64 }],
    ));
...
fn arm_and_emits_ands() {
    let out = emit(&single_block_word(
        sig_0_1(TY_I64),
        &[OpKind::ConstI64(1), OpKind::ConstI64(0), OpKind::AndBool, OpKind::Drop { ty: TY_I64 }, OpKind::Drop { ty: TY_I64 }],
    ));
...
fn arm_smoke_golden() {
    ...
    assert!(out.contains("w_") || out.contains(".thumb_func"), "must label the word, got: {out}");
