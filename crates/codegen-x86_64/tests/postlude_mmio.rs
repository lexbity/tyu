...
//! same descriptor-sourced emulated aperture as the bounds check (P3, FR-21;
...
/// Emit an MMIO word + postlude against a single emulated aperture of `size`.
fn postlude_with_emulated(size: u32) -> String {
    ...
    backend
        .set_mmio_apertures(&[MmioApertureSpec {
            id: 0,
            name: atom(b"mmio"),
...
fn reservation_size_matches_emulated_aperture() {
    run_8mb!({
        ...
        assert!(
            ...
            "0x10000 aperture must reserve 65536 bytes, got:\n{out}"
        );
    });
}

#[test]
fn mmio_without_emulated_aperture_is_loud_error() {
    run_8mb!({
        let mod_ast = empty_module(b"module m; end;");
        let mut out = TestOut::new();
        let mut backend =
            X86_64HostedBackend::new(&mod_ast, b"", &mut out, false, AsmMode::Executable);
        backend.set_mmio_apertures(&[]).unwrap();
        // No declared aperture: the bounds check must fail loudly (NoMmioAperture)
        // rather than lower against an implicit default.
        let err = backend.emit_word(&mmio_store_word()).unwrap_err();
        assert!(matches!(err, CodegenError::NoMmioAperture), "got: {err:?}");
    });
}
